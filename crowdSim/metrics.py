...
class MetricsManager:
    ...
    def calculate_scores(self, total_cells, dt_step_avg):
        ...
        if total_agents > 0:
            t_90_idx = next(
                (
                    i
                    for i, x in enumerate(self.exited_counts)
                    if x >= 0.9 * total_agents
                ),
                -1,
            )
        ...
        rho_over = (
            (sum(self.overloaded_counts) / total_space_time)
            if total_space_time > 0
            else 0
        )
        ...
        for i in range(len(self.agent_travel_dists)):
            if self.agent_travel_dists[i] > 0 and self.agent_straight_dists[i] > 0:
                detour_indices.append(
                    self.agent_travel_dists[i] / self.agent_straight_dists[i]
                )
        D_mean = np.mean(detour_indices) if detour_indices else 1.0

        dt_step_ms = dt_step_avg * 1000.0

        def clamp01(x):
            return max(0.0, min(1.0, x))

        score_T_total = clamp01((300 - T_total) / 180)
        score_t90 = clamp01((200 - t_90) / 150)

        score_rho_max = clamp01((6.0 - rho_max) / 4.0)
        score_rho_over = clamp01((0.30 - rho_over) / 0.30)
        score_collisions = clamp01((800000 - N_collisions) / 700000)

        # Comfort
        score_d_min = clamp01((d_min_avg - 0.6) / 0.6)
        score_detour = clamp01((3.2 - D_mean) / 2.2)

        score_perf = clamp01((100 - dt_step_ms) / 90)

        SafetyScore = (
            0.45 * score_rho_max + 0.25 * score_rho_over + 0.30 * score_collisions
        )

        EfficiencyScore = 0.55 * score_T_total + 0.45 * score_t90

        ComfortScore = 0.50 * score_detour + 0.50 * score_d_min
    ...
    def plot_dashboard(self, scores=None, save_path=None):
        ...
        if valid_times:
            ...
            ax6.axvline(
                mean_time, color="b", linestyle="--", label=f"Mean: {mean_time:.1f}s"
            )
            ax6.axvline(
                p95_time, color="r", linestyle="--", label=f"95%: {p95_time:.1f}s"
            )
            ax6.legend()
        ...
        if valid_dists:
            ...
            ax7.axvline(
                mean_dist, color="b", linestyle="--", label=f"Mean: {mean_dist:.1f}m"
            )
            ax7.legend()
